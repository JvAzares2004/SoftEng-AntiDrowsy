...
function AdminSignUp() {
        ...
        const timer = setTimeout(async () => {
            if (formData.email && formData.email.length > 0) {
                ...
                if (!emailRegex.test(formData.email)) {
                    ...
                    try {
                        const response = await fetch(`${API_URL}/admin/auth/check-email`, {
                            method: 'POST',
                            headers: {
        ...
        const timer = setTimeout(async () => {
            if (formData.contactNumber && formData.contactNumber.length > 0) {
                ...
                if (!phoneRegex.test(formData.contactNumber)) {
                    ...
                    try {
                        const response = await fetch(`${API_URL}/admin/auth/check-contact`, {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                            },
    ...
    const handleSignUp = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (data.success) {
                // Navigate to admin login page after successful signup
        ...
        <div className="flex flex-col inter min-h-screen bg-gray-50">
            ...
            <div className="flex flex-col justify-center items-center flex-1 p-4 py-8">
                <div className="flex flex-col gap-4 bg-[#C52233] px-8 py-8 md:px-10 rounded-xl w-full max-w-md shadow-lg">
                    ...
                    <h1 className="text-white text-2xl font-semibold text-center">Create Admin Account</h1>
