    ...
    const fetchProfile = async () => {
      try {
        ...
        const response = await fetch(
          `${API_URL}/auth/profile?email=${encodeURIComponent(user.email)}&role=${userRole}`,
        );
  ...
  const confirmChange = async () => {
    ...
    switch (editingField) {
      ...
      case "contactNumber": {
        ...
        if (!phoneRegex.test(editingValue)) {
          ...
          setModalType("error");
          setModalMessage(
            "Contact number can only contain numbers and phone characters (+, -, spaces, parentheses)",
          );
          setShowResultModal(true);
          return;
        }
    ...
    try {
      ...
      if (data.success) {
        ...
        setModalMessage(
          `${editingField.charAt(0).toUpperCase() + editingField.slice(1).replace(/([A-Z])/g, " $1")} updated successfully!`,
        );
        setShowResultModal(true);
      } else {
        setShowEditModal(false);
            ...
            <div className='flex flex-col gap-2'>
              ...
              <div className='flex gap-2 items-center'>
                <input
                  ...
                  value={
                    passwordLength
                      ? "•".repeat(Math.min(passwordLength, 20))
                      : ""
                  }
                  readOnly
                  placeholder={
                    passwordLength ? `${passwordLength} characters` : "Not set"
                  }
                  className='flex-1 px-4 py-3 border border-gray-300 rounded-lg bg-gray-50 text-gray-700'
                />
                <button
        ...
        <div className='flex justify-end pb-8'>
          ...
          >
            ...
            >
              ...
              <path
                fillRule='evenodd'
                d='M13 10a1 1 0 011-1h3.586l-1.293-1.293a1 1 0 111.414-1.414l3 3a1 1 0 010 1.414l-3 3a1 1 0 01-1.414-1.414L17.586 11H14a1 1 0 01-1-1z'
                clipRule='evenodd'
              />
            </svg>
            Logout
            ...
            <div className='flex flex-col'>
              <h3 className='text-xl font-bold mb-4 text-gray-800'>
                ...
                {editingField.charAt(0).toUpperCase() +
                  editingField.slice(1).replace(/([A-Z])/g, " $1")}
              </h3>
                ...
                {editingField === "password" ? (
                  <div className='flex flex-col gap-4'>
                    ...
                    <div className='text-xs space-y-1'>
                      ...
                      <p
                        className={
                          editingValue.length >= 6
                            ? "text-green-600"
                            : "text-gray-500"
                        }
                      >
                        {editingValue.length >= 6 ? "✓" : "○"} At least 6
                        characters
                      </p>
                      <p
                        className={
                          /[A-Z]/.test(editingValue)
                            ? "text-green-600"
                            : "text-gray-500"
                        }
                      >
                        {/[A-Z]/.test(editingValue) ? "✓" : "○"} One uppercase
                        letter
                      </p>
                      <p
                        className={
                      ...
                      >
                        {/[a-z]/.test(editingValue) ? "✓" : "○"} One lowercase
                        letter
                      </p>
                      <p
                        className={
                          /[0-9]/.test(editingValue)
                            ? "text-green-600"
                            : "text-gray-500"
                        }
                      >
                      ...
                      <p
                        className={
                          ...
                          editingValue === confirmPasswordValue
                            ? "text-green-600"
                            : "text-gray-500"
                        }
                      >
        ...
        <div className='fixed inset-0 z-[60] flex items-center justify-center bg-black/50'>
          <div className='bg-white rounded-lg p-6 shadow-xl max-w-md w-full mx-4'>
            <div className='flex flex-col items-center'>
              ...
              <h3
                className={`text-xl font-bold mb-2 ${modalType === "success" ? "text-green-600" : "text-red-600"}`}
              >
